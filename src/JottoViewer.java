 ...
 * @version August 2016, unsused code removal and switch to JSON
 */
public class JottoViewer extends JFrame {
    ...
    protected void clearState() {
        for (int k = myGuesses.length - 1; k >= 0; k--) {
            myGuesses[k].setUserGuesses(false);
            myGuesses[k].clear();
    ...
    public void getGuessFromState(int common) {
    	
    	if (common == 6) {
    		this.showModalInfo("You win!!!");
    		setStopped();
    		return;
    	}
        Map<Integer,String> map = new HashMap<Integer,String>();
        List<Jot> jots = new ArrayList<Jot>();
    	for(int k=0; k < myCurrentGuess; k++){
    		String s = myGuesses[k].getString();
    		int count = myGuesses[k].getCorrect();
    		
    		String combo = s+"-"+count;
    		map.put(k, combo);
    		jots.add(new Jot(k,s,count));
    	}
    	...
    	if (s.length() != 5) {
    		this.setStopped();
    		this.showError("you've made a mistake or I don't know your word");
    		return;
    	}
    	
    	myGuesses[myCurrentGuess].setResponse(s);
    	this.myCurrentGuess++;
    	System.out.println("just set guess "+myCurrentGuess);
    }
    ...
    protected class JottoPanel extends JPanel {
        ...
        public JottoPanel() {
            ...
            setUserGuesses(false);
            
            myGuessField.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    // nothing for word, old;
                }

            }); 
            myCorrectField.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                	int common = Integer.parseInt(e.getActionCommand());
                	getGuessFromState(common);
                }
        ...
        public String getString(){
        	return myGuessField.getText();
        }
        public int getCorrect(){
        	return Integer.parseInt(myCorrectField.getText());
        }
        ...
        public void setResponse(String s) {
            myFields[0].setText(s);
            myFields[0].setEnabled(false);
            myFields[1].setEnabled(true);
            myFields[1].requestFocus();
